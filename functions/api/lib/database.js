...
export const StudentService = {
  ...
  async recalculateBalance(db, studentId) {
    // Get student with course price
    const student = await this.findById(db, studentId);
    if (!student) throw new DatabaseError('Student not found');
    ...
    // Update student balance and total_paid
    const query = `UPDATE students SET balance = ?, total_paid = ? WHERE id = ?`;
    return await executeUpdate(db, query, [newBalance, totalPaid, studentId]);
  }
};
