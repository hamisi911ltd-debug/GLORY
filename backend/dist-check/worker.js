...
var Router = class {
  ...
  add(method, pattern, handler) {
    const keys = [];
    const regexStr = pattern.replace(/\//g, "\\/").replace(/:([a-zA-Z_]+)/g, (_, key) => {
      keys.push(key);
      return "([^/]+)";
    });
  ...
  async handle(request, env2, ctx) {
    ...
    for (const route of this.routes) {
      ...
      return route.handler(req, env2, ctx);
    }
...
// src/routes/auth.js
function registerAuthRoutes(router2) {
  router2.post("/api/auth/register", async (req, env2) => {
    ...
    const admin = getAdminClient(env2);
    ...
    if (error3) return badRequest(error3.message);
  ...
  router2.get("/api/auth/me", async (req, env2) => {
    const authHeader = req.headers.get("Authorization") ?? "";
    if (!authHeader.startsWith("Bearer ")) return unauthorized();
    const token = authHeader.slice(7);
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.auth.getUser(token);
    if (error3 || !data?.user) return unauthorized("Invalid token");
...
async function getUserRoles(userId, env2) {
  const admin = getAdminClient(env2);
  const { data } = await admin.from("user_roles").select("role").eq("user_id", userId);
  return (data ?? []).map((r) => r.role);
}
__name(getUserRoles, "getUserRoles");
async function checkRole(userId, env2, ...requiredRoles) {
  const roles = await getUserRoles(userId, env2);
...
function registerProfileRoutes(router2) {
  router2.get("/api/profile", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("profiles").select("*").eq("id", auth.user.id).single();
    if (error3) return notFound("Profile not found");
    return ok({ profile: data });
  });
  router2.patch("/api/profile", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    ...
    if (branch_id !== void 0) updates.branch_id = branch_id;
    if (avatar_url !== void 0) updates.avatar_url = avatar_url;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("profiles").update(updates).eq("id", auth.user.id).select().single();
    if (error3) return badRequest(error3.message);
    return ok({ profile: data });
  });
  router2.post("/api/profile/change-password", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    ...
    const admin = getAdminClient(env2);
    const { error: error3 } = await admin.auth.admin.updateUserById(auth.user.id, {
      password: newPassword
    });
    if (error3) return badRequest(error3.message);
    return ok({ message: "Password updated successfully" });
  });
  router2.get("/api/profile/preferences", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    return ok({
  ...
  router2.put("/api/profile/preferences", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
...
__name(registerProfileRoutes, "registerProfileRoutes");

// src/routes/students.js
function registerStudentRoutes(router2) {
  router2.get("/api/students/me", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("students").select("*, course:courses(name, vehicle_type, lesson_count)").eq("user_id", auth.user.id).single();
    if (error3) return notFound("Student record not found");
    return ok({ student: data });
  });
  router2.get("/api/students", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "branch_admin", "super_admin", "instructor", "examiner");
    if (denied) return denied;
    const { branch_id, status, search } = req.query;
    const admin = getAdminClient(env2);
    let query = admin.from("students").select("*, user:profiles(full_name, phone, avatar_url), course:courses(name, vehicle_type)").order("enrolled_at", { ascending: false });
  ...
  router2.get("/api/students/:id", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "branch_admin", "super_admin", "instructor", "examiner");
    if (denied) return denied;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("students").select("*, user:profiles(*), course:courses(*)").eq("id", req.params.id).single();
    if (error3) return notFound("Student not found");
    return ok({ student: data });
  });
  router2.patch("/api/students/:id", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "branch_admin", "super_admin", "examiner");
    if (denied) return denied;
    const { status, progress_pct, exam_ready } = req.body ?? {};
    const updates = {};
    if (status !== void 0) updates.status = status;
    if (progress_pct !== void 0) updates.progress_pct = progress_pct;
    if (exam_ready !== void 0) updates.exam_ready = exam_ready;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("students").update(updates).eq("id", req.params.id).select().single();
    if (error3) return badRequest(error3.message);
    return ok({ student: data });
  });
}
__name(registerStudentRoutes, "registerStudentRoutes");

// src/routes/lessons.js
function registerLessonRoutes(router2) {
  router2.get("/api/lessons", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const roles = await getUserRoles(auth.user.id, env2);
    const isStaff = roles.some((r) => ["branch_admin", "super_admin", "instructor"].includes(r));
    const { status, limit = "20", offset = "0" } = req.query;
    const admin = getAdminClient(env2);
    let query = admin.from("lessons").select("*, instructor:instructors(id, user_id), vehicle:vehicles(plate, type)").order("scheduled_at", { ascending: false }).range(Number(offset), Number(offset) + Number(limit) - 1);
    if (!isStaff) {
  ...
  router2.post("/api/lessons", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    ...
    const admin = getAdminClient(env2);
    const { data: student, error: studentErr } = await admin.from("students").select("id").eq("user_id", auth.user.id).single();
    if (studentErr || !student) {
      return badRequest("No student record found for this user");
    }
    const { data: existing } = await admin.from("lessons").select("id").eq("instructor_id", instructor_id).eq("scheduled_at", scheduled_at).neq("status", "cancelled").single();
    if (existing) return conflict("This instructor slot is already booked");
    const { data, error: error3 } = await admin.from("lessons").insert({ student_id: student.id, instructor_id, vehicle_id: vehicle_id ?? null, scheduled_at, status: "pending" }).select().single();
    if (error3) return badRequest(error3.message);
    return created({ lesson: data });
  });
  router2.patch("/api/lessons/:id", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    ...
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("lessons").update(updates).eq("id", req.params.id).select().single();
    if (error3) return badRequest(error3.message);
    return ok({ lesson: data });
  });
}
__name(registerLessonRoutes, "registerLessonRoutes");

// src/routes/payments.js
function registerPaymentRoutes(router2) {
  router2.get("/api/payments", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const roles = await getUserRoles(auth.user.id, env2);
    const isFinance = roles.some(
      (r) => ["finance", "branch_admin", "super_admin"].includes(r)
    );
    const admin = getAdminClient(env2);
    let query = admin.from("payments").select("*").order("created_at", { ascending: false });
    if (!isFinance) {
      const { data: student } = await admin.from("students").select("id").eq("user_id", auth.user.id).single();
  ...
  router2.post("/api/payments/mpesa/initiate", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    ...
    const { data, error: error3 } = await admin.from("payments").insert({
  ...
  router2.post("/api/payments/mpesa/callback", async (req, env2) => {
    const body = req.body ?? {};
  ...
  router2.post("/api/payments/cash", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "finance", "branch_admin", "super_admin");
    if (denied) return denied;
...
__name(registerPaymentRoutes, "registerPaymentRoutes");

// src/routes/notifications.js
function registerNotificationRoutes(router2) {
  router2.get("/api/notifications", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const { read, limit = "20" } = req.query;
    const admin = getAdminClient(env2);
    let query = admin.from("notifications").select("*").eq("user_id", auth.user.id).order("created_at", { ascending: false }).limit(Number(limit));
    if (read !== void 0) query = query.eq("read", read === "true");
    const { data, error: error3 } = await query;
    if (error3) return badRequest(error3.message);
    return ok({ notifications: data ?? [] });
  });
  router2.patch("/api/notifications/:id/read", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("notifications").update({ read: true }).eq("id", req.params.id).eq("user_id", auth.user.id).select().single();
    if (error3) return badRequest(error3.message);
    return ok({ notification: data });
  });
  router2.post("/api/notifications/read-all", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const admin = getAdminClient(env2);
    const { error: error3 } = await admin.from("notifications").update({ read: true }).eq("user_id", auth.user.id).eq("read", false);
    if (error3) return badRequest(error3.message);
    return ok({ message: "All notifications marked as read" });
  });
}
__name(registerNotificationRoutes, "registerNotificationRoutes");

// src/routes/staff.js
function registerStaffRoutes(router2) {
  router2.get("/api/staff/branch/stats", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "branch_admin", "super_admin");
    if (denied) return denied;
    const admin = getAdminClient(env2);
    ...
    const [studentsRes, lessonsRes, paymentsRes, vehiclesRes] = await Promise.all([
      ...
      admin.from("lessons").select("id", { count: "exact", head: true }).gte("scheduled_at", todayStart),
  ...
  router2.get("/api/staff/fleet", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "branch_admin", "super_admin");
    if (denied) return denied;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("vehicles").select("*").order("plate");
    if (error3) return badRequest(error3.message);
    return ok({ vehicles: data ?? [] });
  });
  router2.post("/api/staff/fleet", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "branch_admin", "super_admin");
    if (denied) return denied;
    ...
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("vehicles").insert({ plate, type, year, branch_id, status: "available" }).select().single();
    if (error3) return badRequest(error3.message);
    return created({ vehicle: data });
  });
  router2.patch("/api/staff/fleet/:id", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "branch_admin", "super_admin");
    if (denied) return denied;
    ...
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("vehicles").update(updates).eq("id", req.params.id).select().single();
    if (error3) return badRequest(error3.message);
    return ok({ vehicle: data });
  });
  router2.get("/api/staff/instructor/schedule", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "instructor", "branch_admin", "super_admin");
    if (denied) return denied;
    const admin = getAdminClient(env2);
    const { data: instructor } = await admin.from("instructors").select("id").eq("user_id", auth.user.id).single();
    if (!instructor) return notFound("Instructor record not found");
    const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const { data, error: error3 } = await admin.from("lessons").select("*, student:students(id, user:profiles(full_name)), vehicle:vehicles(plate, type)").eq("instructor_id", instructor.id).gte("scheduled_at", `${today}T00:00:00`).lte("scheduled_at", `${today}T23:59:59`).order("scheduled_at");
    if (error3) return badRequest(error3.message);
    return ok({ lessons: data ?? [] });
  });
  router2.get("/api/staff/examiner/students", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "examiner", "super_admin");
    if (denied) return denied;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("students").select("*, user:profiles(full_name), course:courses(name, vehicle_type)").eq("exam_ready", false).order("enrolled_at");
    if (error3) return badRequest(error3.message);
    return ok({ students: data ?? [] });
  });
  router2.post("/api/staff/examiner/assess/:studentId", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "examiner", "super_admin");
    if (denied) return denied;
    const { mock_score, exam_ready } = req.body ?? {};
    if (mock_score === void 0 || exam_ready === void 0) {
      return badRequest("mock_score and exam_ready are required");
    }
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("students").update({ exam_ready, progress_pct: mock_score }).eq("id", req.params.studentId).select().single();
    if (error3) return badRequest(error3.message);
    return ok({ student: data });
  });
  router2.get("/api/staff/platform/stats", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "super_admin");
    if (denied) return denied;
    const admin = getAdminClient(env2);
    const monthStart = new Date(
      (/* @__PURE__ */ new Date()).getFullYear(),
    ...
    const [studentsRes, revenueRes, lessonsRes, instructorsRes, branchesRes, certsRes] = await Promise.all([
      admin.from("students").select("id", { count: "exact", head: true }),
      admin.from("payments").select("amount").eq("status", "paid").gte("created_at", monthStart),
  ...
  router2.get("/api/staff/users", async (req, env2) => {
    const auth = await authenticate(req.raw, env2);
    if (auth.error) return auth.error;
    const denied = await checkRole(auth.user.id, env2, "super_admin");
    if (denied) return denied;
    const admin = getAdminClient(env2);
    const { data, error: error3 } = await admin.from("profiles").select("*, user_roles(role)").order("created_at", { ascending: false });
    if (error3) return badRequest(error3.message);
    return ok({ users: data ?? [] });
  });
}
__name(registerStaffRoutes, "registerStaffRoutes");
...
function corsHeaders(env2, requestOrigin) {
  const allowed = env2.FRONTEND_URL ?? "https://driveschool.vercel.app";
...
function addCors(response, env2, requestOrigin) {
  const headers = new Headers(response.headers);
  for (const [k, v] of Object.entries(corsHeaders(env2, requestOrigin))) {
    headers.set(k, v);
  }
  ...
  async fetch(request, env2, ctx) {
    ...
    if (request.method === "OPTIONS") {
      return new Response(null, {
        status: 204,
        headers: corsHeaders(env2, origin)
      });
    }
    const url = new URL(request.url);
    ...
    let response;
    try {
      response = await router.handle(request, env2, ctx);
    } catch (err) {
    ...
    return addCors(response, env2, origin);
  }
};
export {
